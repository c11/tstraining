...
//------------------------------------------------------------	
	...
	private var _historyVertices:ArrayCollection = new ArrayCollection();
	public function get historyVertices():ArrayCollection {
		return _historyVertices;
	}
	
	public function set historyVertices(value:ArrayCollection):void {
		_historyVertices = value;
	...
	public var historySegments:ArrayCollection;
	...
	public function Plot(plotid:int=0)
	{
		...
		historySegments = new ArrayCollection();
	...
	private function updateSegments():void {
		...
		for (var i:int = 1; i<vertices.length; i++) {
			...
			var seg:TsSegment = new TsSegment(this.project_id, this.tsa, plotid, vv.interpreter, 
				vertices[i-1], vertices[i]);
			segments.addItem(seg);
		}
	}
	 ...
	 * rebuild segments from vertices
	 */
	public function updateHistorySegments():void {
		historySegments.removeAll();
		for (var i:int = 1; i<_historyVertices.length; i++) {
			var vv:Vertex = _historyVertices[i-1] as Vertex;
			var seg:TsSegment = new TsSegment(this.project_id, this.tsa, plotid, vv.interpreter, _historyVertices[i-1], _historyVertices[i]);
			historySegments.addItem(seg);
		}
	}
